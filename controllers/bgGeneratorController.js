...
exports.deleteBackground = async (req, res) => {
  try {
    const { id } = req.params;
    await pool.query('DELETE FROM generated_backgrounds WHERE id = ? AND user_id = ?', [id, req.user.id]);
...
exports.saveBackground = async (req, res) => {
  try {
    ...
    const [bg] = await pool.query('SELECT * FROM generated_backgrounds WHERE id = ? AND user_id = ?', [bg_id, req.user.id]);
    if (!bg.length) {
      return res.status(404).json({ error: 'Background not found' });
    }

    // Check if already saved
    const [existing] = await pool.query('SELECT id FROM saved_backgrounds WHERE user_id = ? AND background_id = ?', [req.user.id, bg_id]);
    if (existing.length) {
      return res.json({ success: true, message: 'Already saved' });
    }

    await pool.query('INSERT INTO saved_backgrounds (user_id, background_id) VALUES (?, ?)', [req.user.id, bg_id]);
